...
export default function Tree({ structure, colors, hide, arrow }) {
    ...
    return <div className={classNames({ "opacity-10": hide })}>
        {arrow === "left" &&
            <div className="text-black text-lg absolute left-5 rounded-full p-2 text-center"><FontAwesomeIcon icon={faHandPointLeft} className="align-middle w-20 h-20" style={{color:  "grey"}} /></div>
        }
        {arrow === "right" &&
            <div className="text-black text-lg absolute right-5 rounded-full p-2 text-center"><FontAwesomeIcon icon={faHandPointRight} className="align-middle w-20 h-20" style={{color:  "grey"}}/></div>
        }
        <AnimateSharedLayout>
